...
===  Ashish Execute pipelines manually from the command line for your custom built Collections
...
. By default the pipelines assume you are pulling the collection from `kabanero` so it looks for the collection image in dockerhub repository. If you want to use

==== For Collection image stored with internal openshift registry route url on the OCP cluster

==== For Collection image stored with external container registry route url
