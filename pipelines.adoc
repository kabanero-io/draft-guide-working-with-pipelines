...
== Execute pipelines manually from the command line for your custom built Collections

If you have your own custom build collection image and want to run any pipelines against it instead of out of box kabanero collections, then you need to follow below steps as per the required scenarios.

=== Setup of container registry url for the custom Collection image

By default, the pipelines pull the collection images for docker hub. If you are publishing your collection images to any other repository, follow the instructions below to configure the custom repository that the pipelines should pull the collection images from.

. After you have cloned kabanero-pipelines repository find the configmap template file `collection-image-registry-map.yaml` and add your container registry URL in it for the key `default-collection-image-registry-url` 


 cd kabanero-pipelines/pipelines/common/
 vi collection-image-registry-map.yaml

. Apply the above configmap file which will set your container registry.

 oc apply -f collection-image-registry-map.yaml

==== For Collection image stored in container registry with internal route url on the OCP cluster

For internal openshift registry you need to only setup above `collection-image-registry-map.yaml` with the internal registry url.

NOTE : The service account should let the pipelines pull from the internal registry without needing to configure a secret.

==== For Collection image stored in container registry with external route url
...
. Find the template file `default-collection-image-registry-secret.yaml` in cloned kabanero-pipelines repo and update it with the username and token password for the container registry url you specified above.

 cd kabanero-pipelines/pipelines/common

 
. You need to first create `base64` format of the username and password for the container registry url.

 echo -n <your-registry-username> | base64
 echo -n <your-registry-password> | base64
 
. Update the file `default-collection-image-registry-secret.yaml` with `base64` format from above step output.
...
. Finally run the pipeline as shown in above section.
